...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const image = await handleImage(img);
    ...
    setLoader(true);
    await dispatch(registerUser(userRegistrationData));
    setLoader(false);
  };
  useEffect(() => {
    if (user.loginStatus === "success" && user.user) {
      setUserDetails({
        name: "",
        surname: "",
        email: "",
  ...
  }, [user.loginStatus, user.user, navigate]);
  return (
        ...
        <section className="w-full h-full  py-5 flex justify-center items-center">
          ...
          >
            ...
            <div className="flex flex-col my-3 gap-2">
              <label htmlFor="img">Choose Your Photo</label>
              <input
